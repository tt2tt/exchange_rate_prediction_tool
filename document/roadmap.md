...
- [〇] 入力値検証とエラー表示（カラム不足、行数不足、範囲外値）を連携
- [〇] 実行フロー連携：前処理→特徴量→学習→評価→バックテスト→出力
- [〇] 進捗バー・ログ表示・結果セクション（予測、検証、バックテスト）を実装
- [〇] （任意）終値・MAとシグナルのミニチャート描画を実装（`ui/main_window.py`で描画ロジックを追加）
